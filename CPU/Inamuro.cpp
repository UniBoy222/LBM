...
void Inamuro::solvePressurePoisson()
{
    ...
    const std::uint64_t prev_iteration_count = last_iteration_count;
    ...
    std::cout << "[PressureSolve] 调用=" << pressure_solve_call_count
              ...
              << " strict_unbounded=yes check_interval=100 tolerance=1e-3"
              << std::endl;
    ...
    while (true)
    {
        if (iteration_count == std::numeric_limits<std::uint64_t>::max())
        ...
        if (iteration_count % convergence_check_interval == 0)
        {
            ...
            if (!std::isfinite(gauge_shift))
                throw std::runtime_error("pressure gauge is NaN/Inf");
            for (int x = 0; x < lx; ++x)
                for (int y = 0; y < ly; ++y)
                    for (int z = 0; z < lz; ++z) {
            ...
            if (!std::isfinite(error))
                throw std::runtime_error("Poisson residual is NaN/Inf");
