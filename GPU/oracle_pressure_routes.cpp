...
void write_exact_pressure(const std::filesystem::path& path, const State& state, int step)
{
    std::ofstream out(path, std::ios::binary | std::ios::trunc);
    if (!out) throw std::runtime_error("cannot write exact pressure: " + path.string());
    const char magic[8] = {'C', 'L', 'B', 'M', 'P', 'X', '1', '\0'};
    write_header(out, magic, state, static_cast<std::uint32_t>(step), 1);
    write_values(out, state.p);
    if (!out) throw std::runtime_error("failed writing exact pressure: " + path.string());
}
...
std::vector<double> read_exact_pressure(const std::filesystem::path& path,
                                        int nx, int ny, int nz, int step)
{
    std::ifstream in(path, std::ios::binary);
    if (!in) throw std::runtime_error("cannot read exact pressure: " + path.string());
    const char magic[8] = {'C', 'L', 'B', 'M', 'P', 'X', '1', '\0'};
    const auto h = read_header(in, magic);
    if (h[0] != static_cast<std::uint32_t>(nx) || h[1] != static_cast<std::uint32_t>(ny) ||
        h[2] != static_cast<std::uint32_t>(nz) || h[3] != static_cast<std::uint32_t>(step) || h[4] != 1)
        throw std::runtime_error("exact pressure metadata mismatch");
    std::vector<double> pressure(static_cast<std::size_t>(nx) * ny * nz);
    in.read(reinterpret_cast<char*>(pressure.data()), static_cast<std::streamsize>(pressure.size() * sizeof(double)));
    if (!in || in.peek() != std::ifstream::traits_type::eof()) throw std::runtime_error("bad exact pressure payload");
    return pressure;
}
...
Config parse_args(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--feature-dir") cfg.feature_dir = value();
        else if (arg == "--steps") cfg.steps = std::stoi(value());
        else throw std::runtime_error("unknown argument: " + arg);
    }
    if (cfg.params.empty() || cfg.output.empty() || cfg.steps <= 0 || cfg.steps > 200)
        throw std::runtime_error("invalid --params/--output/--steps");
    if (cfg.route != "baseline" && cfg.route != "5" && cfg.route != "3" &&
        cfg.route != "1" && cfg.route != "p-only")
        throw std::runtime_error("--route must be baseline, 5, 3, 1, or p-only");
    if (cfg.route != "baseline" && cfg.baseline.empty()) throw std::runtime_error("candidate route requires --baseline-dir");
    if (!cfg.feature_dir.empty() && cfg.route != "baseline")
        throw std::runtime_error("--feature-dir is allowed only for development baseline generation");
    return cfg;
}

void write_csv_header(std::ofstream& out)
{
    out << "step,iterations,residual,converged,fallback,compute_ms,oracle_io_ms,e2e_ms,validation_io_ms,reference_write_io_ms,"
...
int main(int argc, char** argv)
{
    try {
        ...
        if (cfg.route == "baseline") {
            std::filesystem::create_directories(cfg.output / "exact_pressure");
        ...
        if (cfg.route == "p-only") {
            InamuroCUDA audit_gpu(host);
            audit_gpu.setOracleRoute(InamuroCUDA::OracleRoute::POnlyHotStart);
            const auto pressure = read_exact_pressure(
                cfg.baseline / "exact_pressure" / step_name("p_exact_", 1), nx, ny, nz, 1);
            audit_gpu.uploadOraclePressure(pressure);
            const auto audit = audit_gpu.auditPOnlyInjection();
            std::ofstream audit_json(cfg.output / "p_only_injection_audit.json", std::ios::trunc);
            if (!audit_json) throw std::runtime_error("cannot open p-only injection audit output");
            audit_json << "{\n"
                       << "  \"unchanged_values_checked\": " << audit.unchanged_values_checked << ",\n"
                       << "  \"unchanged_value_mismatches\": " << audit.unchanged_value_mismatches << ",\n"
                       << "  \"pressure_values_checked\": " << audit.pressure_values_checked << ",\n"
                       << "  \"pressure_value_mismatches\": " << audit.pressure_value_mismatches << ",\n"
                       << "  \"passed\": " << (audit.passed ? "true" : "false") << "\n}\n";
            if (!audit.passed) throw std::runtime_error("p-only injection bitwise audit failed");
        }
        InamuroCUDA gpu(host);
        if (!cfg.feature_dir.empty()) {
            std::filesystem::create_directories(cfg.feature_dir);
            gpu.setPoissonFeatureDumpDirectory(cfg.feature_dir);
        }
        gpu.setUseOnePassPoisson(true);
        gpu.setUseFusedBoundaryPressure(true);
        gpu.setUsePoissonGraph(true);
        ...
        if (cfg.route == "5") gpu.setOracleRoute(InamuroCUDA::OracleRoute::ExactPressure);
        else if (cfg.route == "3") gpu.setOracleRoute(InamuroCUDA::OracleRoute::FirstCheckPressurePrev);
        else if (cfg.route == "1") gpu.setOracleRoute(InamuroCUDA::OracleRoute::ExactPressureToHH);
        else if (cfg.route == "p-only") gpu.setOracleRoute(InamuroCUDA::OracleRoute::POnlyHotStart);
        ...
        for (int step = 1; step <= cfg.steps; ++step) {
            double oracle_io_ms = 0.0;
            const auto e2e_start = Clock::now();
            if (cfg.route == "5" || cfg.route == "3" || cfg.route == "1" || cfg.route == "p-only") {
                const auto io_start = Clock::now();
                const auto pressure = read_exact_pressure(cfg.baseline / "exact_pressure" / step_name("p_exact_", step), nx, ny, nz, step);
                gpu.uploadOraclePressure(pressure);
                oracle_io_ms = milliseconds(io_start, Clock::now());
            }
            ...
            if (!diagnostics.converged || diagnostics.fallback_count != 0 ||
            ...
            std::vector<double> hh; gpu.downloadHH(hh);
            const State state = host.capture();
            ...
            if (cfg.route == "baseline") {
                ...
                write_exact_pressure(cfg.output / "exact_pressure" / step_name("p_exact_", step), state, step);
            ...
            const Comparison result = compare(state, reference, hh, host.phase_threshold());
            csv << step << ',' << diagnostics.iterations << ',' << diagnostics.relative_error << ','
                << diagnostics.converged << ',' << diagnostics.fallback_count << ',' << compute_ms << ','
                << oracle_io_ms << ',' << e2e_ms << ',' << validation_io_ms << ',' << reference_write_io_ms << ','
            ...
            csv.flush();
            std::cout << "route=" << cfg.route << " step=" << step
