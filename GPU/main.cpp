...
struct RunConfig {
    ...
    std::uint64_t poisson_iteration_limit = 0;
...
void print_usage(const char* argv0)
{
    std::cout
        ...
        << "       [--poisson-iteration-limit 0] (strict unbounded production)\n"
...
RunConfig parse_args(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "--poisson-iteration-limit") {
            cfg.poisson_iteration_limit = std::stoull(require_value(arg));
    ...
    if (cfg.poisson_check_interval != 100) {
        throw std::runtime_error("strict production requires --poisson-check-interval 100");
    }
    if (cfg.poisson_tolerance != 1.0e-3) {
        throw std::runtime_error("strict production requires --poisson-tolerance 1e-3");
    }
    if (cfg.poisson_iteration_limit != 0) {
        throw std::runtime_error("strict production requires --poisson-iteration-limit 0 (unbounded)");
    }
...
int main(int argc, char** argv)
{
    try {
        ...
        std::cout << "LBM runner mode=" << cfg.mode
                  ...
                  << " poisson_iteration_limit=" << cfg.poisson_iteration_limit
        ...
        if (cfg.mode == "cpu") {
            if (cfg.poisson != "split" || cfg.pressure_boundary != "split") {
                throw std::runtime_error(
                    "strict CPU baseline supports only split Poisson and split pressure boundary");
            }
