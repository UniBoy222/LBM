...
void print_usage(const char* argv0)
{
    std::cout
        ...
        << "       [--poisson-iteration-limit N] (0 means unbounded)\n"
...
RunConfig parse_args(int argc, char** argv)
{
    ...
    if (cfg.poisson_iteration_limit != 0 &&
        cfg.poisson_iteration_limit % static_cast<std::uint64_t>(cfg.poisson_check_interval) != 0) {
        throw std::runtime_error("--poisson-iteration-limit must be 0 or a multiple of --poisson-check-interval");
    }
