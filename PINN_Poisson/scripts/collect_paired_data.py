#!/usr/bin/env python3
"""Collect pre-Poisson inputs paired with post-Poisson pressure targets."""
...
def parse_step_list(text: str | None, final_step: int) -> list[int]:
    ...
    for item in text.split(","):
        ...
        if step <= 0 or step > final_step:
            raise ValueError(f"sample step {step} outside [1, {final_step}]")
        steps.append(step)
...
def main() -> int:
    ...
    label = args.label or f"paired_{args.poisson}_step{args.steps}_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    run_dir = root / "PINN_Poisson" / "data" / "paired_runs" / label
    ...
    cmd = [
        ...
        "--poisson-pair-steps", ",".join(str(step) for step in sample_steps),
        "--no-roofline",
    ]
    ...
    keys = [
        "label", "params", "run_steps", "sample_step", "poisson", "pressure_boundary",
    ...
    with manifest.open("a", newline="") as f:
        ...
        for step in sample_steps:
            input_plt = pair_dir / "pre_poisson" / f"3D{step:09d}.plt"
            target_plt = pair_dir / "post_poisson" / f"3D{step:09d}.plt"
            if not input_plt.exists() or not target_plt.exists():
                raise SystemExit(f"missing pair output for step {step}")
            data = read_tecplot(target_plt)
            fields = data["fields"]  # type: ignore[assignment]
            sample_label = f"{label}_step{step}"
            pressure_bin = pressure_dir / f"{sample_label}_post_pressure.bin"
            ...
            row.update(perf)
            writer.writerow(row)
            print(f"paired {input_plt} -> {target_plt}")
